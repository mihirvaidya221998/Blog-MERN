...
export default function DashPosts() {
  ...
  const handleShowMore = async() =>{
    const startLength = userPosts.length;
    try {
      const res = await fetch(`/api/post/get-posts?userId=${currentUser._id}&startIndex=${startLength}`);
      const data = await res.json();
      if(res.ok){
  ...
  const handleDeletePostButton = async() =>{
    setShowModal(false);
    try {
      const res = await fetch(`/api/post/delete-posts/${postIdToDelete}/${currentUser._id}`, {method: 'DELETE'});
      const data = await res.json();
      if(!res.ok){
            ...
            {userPosts.map((post) =>(
                <Table.Body className='divide-y'>
                  <Table.Row className='bg-white dark:border-gray-500 dark:bg-gray-800'>
                    ...
                    <Table.Cell><Link className='font-medium text-gray-900 dark:text-white' to={`/post/${post.slug}`}>
                      {post.title}
                    </Link></Table.Cell>
                    <Table.Cell><Link to={`/post/${post.slug}`}>
                      {post.category}
                    ...
                    <Table.Cell>
                      <Link className='text-lime-500 hover:underline' to={`update-post/${post._id}`}>
                        <span>Edit</span>
                      </Link>
                    </Table.Cell>
                  </Table.Row>
        ...
        <Modal.Body className={theme === 'dark' ? 'bg-gray-800 text-white' : ''}>
            <div className={`text-center ${theme === 'dark' ? 'dark' : ''}`}>
               ...
               <div className='flex justify-center gap-4'>
                <Button color='failure' onClick={handleDeletePostButton}>Yes</Button>
